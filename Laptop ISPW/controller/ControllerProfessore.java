 ...
 *
 */
	public ArrayList<PrenotazioneEsameBean> visualizzaPrenotazioniAttiveExams(java.util.Date  dataOdierna,String userid,int ore) throws SQLException {
	...
	public ArrayList<PrenotazioneEsameBean> visualizzaStoricoPrenotazioniEsame(java.util.Date inizio,java.util.Date fine,String userid) throws SQLException {
	...
	public PrenotazioneEsameBean RichiestaPrenotazioneEsame(AulaBean aulabean, PrenotazioneEsameBean prenotazionebean) throws SQLException {
		...
		PrenotazioneEsameBean bean=new PrenotazioneEsameBean();
		PrenotazioneEsame entity= new PrenotazioneEsame();
		...
		*/
		Aula aulaperDao=new Aula(aulabean.getTipo(), aulabean.getNumeroPosti(),aulabean.getNome(),aulabean.getProiettore(),aulabean.isMicrofono(),
			...
			value=DaoPrenotazioni.findAuleEsame(aulaperDao);//caratteristiche aule
			for(int i=0;i<value.size();i++) {
				Aula a= new Aula(value.get(i));
				prenotazione.setAula(a);
				value1.add(prenotazione);
				//verifica che tali prenotazioni non esistono per poter prenotare un'aula
				entity=DaoPrenotazioni.findPrenotaEsame(value1.get(i).getAula().getNome(),value1.get(i).getOrario(),(java.sql.Date)value1.get(i).getData());
				if(entity!=null) {
